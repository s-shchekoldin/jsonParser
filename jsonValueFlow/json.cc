...
inline bool json::range_7_1(state_t & state)
{
    ...
    while(state.data < state.end) [[likely]]
    {
        ...
        jsonResult::keyValue(beginData, unsigned(state.data - beginData), !state.consumed, true);
    ...
    if (beginData < state.data)
        jsonResult::keyValue(beginData, unsigned(state.data - beginData), !state.consumed, false);
    state.consumed += unsigned(state.data - beginData);
    state.node = node_t::RANGE_7_1;
    return true;
}
...
inline bool json::range_12_0(state_t & state)
{
    ...
    while(state.data < state.end) [[likely]]
    {
        ...
        jsonResult::value(beginData, unsigned(state.data - beginData), !state.consumed, true);
        state.consumed += unsigned(state.data - beginData);
        state.node = (state.consumed >= 1) ? node_t::LOOP_1_1 : node_t::NO_STATE;
        bool ret = (state.node == node_t::LOOP_1_1);
        state.consumed = 0;
    ...
    if (beginData < state.data)
        jsonResult::value(beginData, unsigned(state.data - beginData), !state.consumed, false);
    state.consumed += unsigned(state.data - beginData);
